...
public class TimeStampVerifier {
     ...
     */
    public TimeStampVerifier(String tsaUrl)
            throws NoSuchAlgorithmException, MalformedURLException, URISyntaxException {
        this(tsaUrl, null, null);
    }

    /**
     * @param tsaUrl The url where TS-Request will be done.
     */
    public TimeStampVerifier(String tsaUrl, String username, String password)
            throws NoSuchAlgorithmException, MalformedURLException, URISyntaxException {
        if (tsaUrl != null) {
            MessageDigest digest = MessageDigest.getInstance("SHA-256");
            this.tsaClient = new TSAClient(new URI(tsaUrl).toURL(), username, password, digest);
        }
    }
