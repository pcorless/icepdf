...
public class TSAClient {
    ...
    /**
     *
     * @param content get token from stream
    ...
    private byte[] getTSAResponse(byte[] request) throws IOException {
        logger.log(Level.FINER, "Opening connection to TSA server");
        ...
        try (OutputStream output = connection.getOutputStream()) {
            ...
            logger.log(Level.SEVERE, "Exception when writing to " + this.url, ex);
            throw ex;
        }

        logger.log(Level.FINER, "Waiting for response from TSA server");

        byte[] response;
        try (InputStream input = connection.getInputStream()) {
            response = input.readAllBytes();
        } catch (IOException ex) {
            logger.log(Level.SEVERE, "Exception when reading from " + this.url, ex);
            throw ex;
        }

        logger.log(Level.FINER, "Received response from TSA server");
