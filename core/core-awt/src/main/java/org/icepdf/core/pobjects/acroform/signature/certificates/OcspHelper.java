...
public class OcspHelper {
    ...
    private void verifyOcspResponse(OCSPResp ocspResponse)
        ...
        if (basicResponse != null) {
            ...
            try {
                checkResponderCertificateUsage(ocspResponderCertificate);
            } catch (CertificateParsingException ex) {
                // unlikely to happen because the certificate existed as an object
                logger.log(Level.SEVERE, "Error parsing responder certificate", ex);
            }
            ...
            if (status instanceof RevokedStatus) {
                ...
                logger.log(Level.FINE, "The certificate was revoked after signing by OCSP {0} on {1}",
                        new Object[]{ocspUrl, revokedStatus.getRevocationTime()});
    ...
    public static void checkResponderCertificateUsage(X509Certificate x509Certificate)
        ...
        if (extendedKeyUsage != null &&
            ...
            logger.log(Level.WARNING, "Certificate extended key usage does not include OCSP responding");
        }
    }
    ...
    private byte[] getKeyHashFromCertHolder(X509CertificateHolder certHolder) {
        ...
        try {
            ...
            // should not happen
            logger.log(Level.WARNING, "SHA-1 Algorithm not found", ex);
            return new byte[0];
        }
    }
    ...
    private void findResponderCertificateByKeyHash(BasicOCSPResp basicResponse, byte[] keyHash)
        ...
        for (X509CertificateHolder certHolder : certHolders) {
            ...
            if (Arrays.equals(keyHash, digest)) {
                try {
                    ...
                    // unlikely to happen because the certificate existed as an object
                    logger.log(Level.WARNING, "Error converting responder certificate", ex);
                }
                break;
            }
        }
        ...
        for (X509Certificate cert : additionalCerts) {
            try {
                ...
                // unlikely to happen because the certificate existed as an object
                logger.log(Level.WARNING, "Error encoding responder certificate", ex);
            }
        }
    }
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_MOVED_TEMP ||
                ...
                if (urlString.startsWith("http://") &&
                    ...
                    logger.finer("redirection to " + location + " followed");
                    return performRequest(location);
                } else {
                    logger.finer("redirection to " + location + " ignored");
                }
            }
        ...
        if (resp != null) {
            ...
            switch (status) {
                case OCSPResponseStatus.INTERNAL_ERROR:
                    ...
                    logger.warning("An internal error occurred in the OCSP Server!");
                    break;
                case OCSPResponseStatus.MALFORMED_REQUEST:
                    ...
                    logger.warning("Your request did not fit the RFC 2560 syntax!");
                    break;
                case OCSPResponseStatus.SIG_REQUIRED:
                    statusInfo = "SIG_REQUIRED";
                    logger.warning("Your request was not signed!");
                    break;
                case OCSPResponseStatus.TRY_LATER:
                    statusInfo = "TRY_LATER";
                    logger.warning("The server was too busy to answer you!");
                    break;
                case OCSPResponseStatus.UNAUTHORIZED:
                    statusInfo = "UNAUTHORIZED";
                    logger.warning("The server could not authenticate you!");
                    break;
                case OCSPResponseStatus.SUCCESSFUL:
                    break;
                default:
                    statusInfo = "UNKNOWN";
                    logger.warning("Unknown OCSPResponse status code! " + status);
            }
        }
